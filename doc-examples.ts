/**
 * Canonical sample datasets and ASCII table renderings for JSDoc documentation.
 * @internal
 * @internalfile
 */

export const DOC_EXAMPLES: Record<string, string> = {
    // 1. Basic 2-column DataFrame (2 rows) - { a: [1, 2], b: ["x", "y"] }
    base_2x2: [
        '>>> const df = $df.data({ a: [1, 2], b: ["x", "y"] })',
        '>>> df',
        'shape: (2, 2)',
        '┌───┬───┐',
        '│ a │ b │',
        '├───┼───┤',
        '│ 1 │ x │',
        '│ 2 │ y │',
        '└───┴───┘'
    ].join('\n     * '),

    // 2. Single 1x1 scalar DataFrame - { val: [42] }
    base_1x1: [
        '>>> const df = $df.data({ val: [42] })',
        '>>> df',
        'shape: (1, 1)',
        '┌─────┐',
        '│ val │',
        '├─────┤',
        '│ 42  │',
        '└─────┘'
    ].join('\n     * '),

    // 3. Basic 3-row numeric DataFrame - { a: [1, 2, 3], b: [10, 20, 30] }
    base_numbers_3x2: [
        '>>> const df = $df.data({ a: [1, 2, 3], b: [10, 20, 30] })',
        '>>> df',
        'shape: (3, 2)',
        '┌───┬────┐',
        '│ a │ b  │',
        '├───┼────┤',
        '│ 1 │ 10 │',
        '│ 2 │ 20 │',
        '│ 3 │ 30 │',
        '└───┴────┘'
    ].join('\n     * '),

    // 4. Single-column numeric 4-row DataFrame - { a: [1, 2, 3, 4] }
    base_numbers_4x1: [
        '>>> const df = $df.data({ a: [1, 2, 3, 4] })',
        '>>> df',
        'shape: (4, 1)',
        '┌───┐',
        '│ a │',
        '├───┤',
        '│ 1 │',
        '│ 2 │',
        '│ 3 │',
        '│ 4 │',
        '└───┘'
    ].join('\n     * '),

    // 5. Nullable 3-row DataFrame - { a: [1, null, 3] }
    base_nulls_3x1: [
        '>>> const df = $df.data({ a: [1, null, 3] })',
        '>>> df',
        'shape: (3, 1)',
        '┌──────┐',
        '│ a    │',
        '├──────┤',
        '│ 1    │',
        '│ null │',
        '│ 3    │',
        '└──────┘'
    ].join('\n     * '),

    // 6. Nullable 3-row 2-column DataFrame - { a: [1, null, 3], b: ["x", "y", null] }
    base_nulls_3x2: [
        '>>> const df = $df.data({ a: [1, null, 3], b: ["x", "y", null] })',
        '>>> df',
        'shape: (3, 2)',
        '┌──────┬──────┐',
        '│ a    │ b    │',
        '├──────┼──────┤',
        '│ 1    │ x    │',
        '│ null │ y    │',
        '│ 3    │ null │',
        '└──────┴──────┘'
    ].join('\n     * '),

    // 7. Categorical / Grouping 3-row DataFrame - { group: ["A", "A", "B"], val: [10, 20, 30] }
    base_grouped_3x2: [
        '>>> const df = $df.data({ group: ["A", "A", "B"], val: [10, 20, 30] })',
        '>>> df',
        'shape: (3, 2)',
        '┌───────┬─────┐',
        '│ group │ val │',
        '├───────┼─────┤',
        '│ A     │ 10  │',
        '│ A     │ 20  │',
        '│ B     │ 30  │',
        '└───────┴─────┘'
    ].join('\n     * '),

    // 8. Array / List column DataFrame for explosion
    base_nested_list: [
        '>>> const df = $df.data({ group: ["A"], values: [[1, 2]] })',
        '>>> df',
        'shape: (1, 2)',
        '┌───────┬────────┐',
        '│ group │ values │',
        '├───────┼────────┤',
        '│ A     │ [1, 2] │',
        '└───────┴────────┘'
    ].join('\n     * '),

    // 9. Two DataFrames for Join operations
    base_join_pair: [
        '>>> const df1 = $df.data({ id: [1, 2], val: ["a", "b"] })',
        '>>> const df2 = $df.data({ id: [1, 2], num: [100, 200] })',
        '>>> df1',
        'shape: (2, 2)',
        '┌────┬─────┐',
        '│ id │ val │',
        '├────┼─────┤',
        '│ 1  │ a   │',
        '│ 2  │ b   │',
        '└────┴─────┘',
        '>>> df2',
        'shape: (2, 2)',
        '┌────┬─────┐',
        '│ id │ num │',
        '├────┼─────┤',
        '│ 1  │ 100 │',
        '│ 2  │ 200 │',
        '└────┴─────┘'
    ].join('\n     * '),

    // 10. Two single-column DataFrames for Concat / HConcat / VConcat
    base_concat_pair: [
        '>>> const df1 = $df.data({ a: [1, 2] })',
        '>>> const df2 = $df.data({ b: [10, 20] })',
        '>>> df1',
        'shape: (2, 1)',
        '┌───┐',
        '│ a │',
        '├───┤',
        '│ 1 │',
        '│ 2 │',
        '└───┘',
        '>>> df2',
        'shape: (2, 1)',
        '┌────┐',
        '│ b  │',
        '├────┤',
        '│ 10 │',
        '│ 20 │',
        '└────┘'
    ].join('\n     * '),

    // 11. Two 1-row DataFrames for minimal Concat examples
    base_concat_1x1_pair: [
        '>>> const df1 = $df.data({ a: [1] })',
        '>>> const df2 = $df.data({ b: [2] })',
        '>>> df1',
        'shape: (1, 1)',
        '┌───┐',
        '│ a │',
        '├───┤',
        '│ 1 │',
        '└───┘',
        '>>> df2',
        'shape: (1, 1)',
        '┌───┐',
        '│ b │',
        '├───┤',
        '│ 2 │',
        '└───┘'
    ].join('\n     * '),

    // 12. Wide DataFrame for Pivot / Unpivot
    base_pivot_table: [
        '>>> const df = $df.data({ year: [2020, 2020, 2021, 2021], month: ["Jan", "Feb", "Jan", "Feb"], revenue: [100, 150, 120, 180] })',
        '>>> df',
        'shape: (4, 3)',
        '┌──────┬───────┬─────────┐',
        '│ year │ month │ revenue │',
        '├──────┼───────┼─────────┤',
        '│ 2020 │ Jan   │ 100     │',
        '│ 2020 │ Feb   │ 150     │',
        '│ 2021 │ Jan   │ 120     │',
        '│ 2021 │ Feb   │ 180     │',
        '└──────┴───────┴─────────┘'
    ].join('\n     * '),

    // 13. Pair of DataFrames for Asof Join (trades & quotes)
    base_asof_pair: [
        '>>> const trades = $df.data([',
        '...   { time: 1000, ticker: "AAPL", price: 150.0 },',
        '...   { time: 1005, ticker: "AAPL", price: 150.5 },',
        '...   { time: 1015, ticker: "AAPL", price: 151.0 }',
        '... ])',
        '>>> const quotes = $df.data([',
        '...   { time: 998, ticker: "AAPL", bid: 149.9 },',
        '...   { time: 1004, ticker: "AAPL", bid: 150.4 },',
        '...   { time: 1010, ticker: "AAPL", bid: 150.8 }',
        '... ])',
        '>>> trades',
        'shape: (3, 3)',
        '┌──────┬────────┬───────┐',
        '│ time │ ticker │ price │',
        '├──────┼────────┼───────┤',
        '│ 1000 │ AAPL   │ 150.0 │',
        '│ 1005 │ AAPL   │ 150.5 │',
        '│ 1015 │ AAPL   │ 151.0 │',
        '└──────┴────────┴───────┘',
        '>>> quotes',
        'shape: (3, 3)',
        '┌──────┬────────┬───────┐',
        '│ time │ ticker │ bid   │',
        '├──────┼────────┼───────┤',
        '│ 998  │ AAPL   │ 149.9 │',
        '│ 1004 │ AAPL   │ 150.4 │',
        '│ 1010 │ AAPL   │ 150.8 │',
        '└──────┴────────┴───────┘'
    ].join('\n     * ')
};
