...
try {
    ...
    const res1 = df1.explode([$df.col("list1"), $df.col("list2")]).toDicts();
    ...
    const resSelectors = df1.explode($df.exclude("id")).toDicts();
    ...
    const explodedEmpty = emptyDf.explode("list").toDicts();
    ...
    const explodedTyped = typedDf.explode("values").toDicts();
    ...
    // 5. Options combinations: emptyAsNull & keepNulls
    const mixedDf = $df.data({
        id: [1, 2, 3, 4],
        arr: [[1, 2], [], null, [3]]
    });

    const defaultExplode = mixedDf.explode("arr").toDicts();
    ...
    const noEmpty = mixedDf.explode("arr", { emptyAsNull: false }).toDicts();
    if (noEmpty.length !== 4) throw new Error(`emptyAsNull=false expected 4 rows, got ${noEmpty.length}`);

    const noNulls = mixedDf.explode("arr", { keepNulls: false }).toDicts();
    if (noNulls.length !== 4) throw new Error(`keepNulls=false expected 4 rows, got ${noNulls.length}`);

    const strictExplode = mixedDf.explode("arr", { emptyAsNull: false, keepNulls: false }).toDicts();
    ...
    console.log("✓ Pass: 5. Explode options (emptyAsNull & keepNulls)");
    ...
    const explodedMix = mixedListsDf.explode("val").toDicts();
    ...
    const singleExploded = singleRowDf.explode("items").toDicts();
    ...
    const explodedInternalNulls = internalNullsDf.explode("val").toDicts();
    ...
    const explodedStructs = structArrayDf.explode("items").toDicts();
    ...
    const explodedDatesBools = datesBoolsDf.explode(["dates", "bools"]).toDicts();
    ...
    const explodedMatrix = nestedArrayDf.explode("matrix").toDicts();
    ...
    const chainedDf = df1.explode("list1").withColumns($df.col("list1").add(100).alias("list1_plus_100")).toDicts();
    ...
    // 17. Exploding in withColumns()
    const withColsDf = df1.withColumns($df.col("list1").arr.explode().alias("exploded_val")).toDicts();
    if (withColsDf.length !== 3) throw new Error(`withColumns explode expected 3 rows, got ${withColsDf.length}`);
    if (withColsDf[0].exploded_val !== 10 || withColsDf[2].exploded_val !== 30) throw new Error("withColumns explode failed");
    console.log("✓ Pass: 17. Exploding in withColumns()");

    // 18. String column str.explode()
    const strDf = $df.data({ word: ["cat", "dog"] });
    const explodedStr = strDf.withColumns($df.col("word").str.explode().alias("chars")).toDicts();
    ...
    const allEmptyDefault = allEmptyDf.explode("list").toDicts();
    ...
    const allEmptyDrop = allEmptyDf.explode("list", { emptyAsNull: false }).toDicts();
    ...
    const allNullDefault = allNullDf.explode("list").toDicts();
    if (allNullDefault.length !== 3) throw new Error(`All null default expected 3 rows of null, got ${allNullDefault.length}`);

    const allNullDrop = allNullDf.explode("list", { keepNulls: false }).toDicts();
    ...
    const multiColExprDf = df1.explode($df.col(["list1", "list2"])).toDicts();
    ...
    const explodedLarge = largeDf.explode("items").toDicts();
