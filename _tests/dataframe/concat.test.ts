...
const collectedTyped = dfTypedConcat.to_dicts();
...
const collectedMixed = dfMixedConcat.to_dicts();
...
if (dfInterspersed.height !== 1 || dfInterspersed.to_dicts()[0].a !== 100) {
...
const diagRows = dfMultiDiag.to_dicts() as any[];
if (
...
const hRows = dfHConcat.to_dicts() as any[];
if (
...
if ((dfZeroStack.to_dicts() as any[])[1].a !== 200 || (dfZeroStack.to_dicts() as any[])[1].b !== 2.5) {
...
const nestedRows = dfNestedDiag.to_dicts() as any[];
if (
...
const bigRows = bigStack.to_dicts() as any[];
...
const massRows = dfMassive.to_dicts() as any[];
...
const tempRows = dfTemporalStack.to_dicts() as any[];
...
const sRows = sparseDiag.to_dicts() as any[];
if (
...
const binRows = dfBinStack.to_dicts() as any[];
...
const f32Rows = dfTypedVariants.to_dicts() as any[];
...
const polyRows = mixedInputConcat.to_dicts() as any[];
...
const chainRows = chainedResult.to_dicts() as any[];
