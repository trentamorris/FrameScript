...
try {
    ...
    const result = dfWithWindow.toDicts();
    ...
    const dfAllExclude = df.select($df.exclude("salary").str.lower()).toDicts();
    ...
    // 9. Test withColumns wildcard select and exclude
    console.log("\nTesting withColumns wildcard select and exclude:");
    const dfWithCols = df.withColumns(
    ...
    if (dfWithCols.length !== 5 || dfWithCols[0].salary !== 1000 || dfWithCols[0].name !== "ALICE" || dfWithCols[0].dept !== "HR") {
        console.error("withColumns wildcard exclude failed!");
        passed = false;
    }

    // 9.5 Test withColumns old record signature
    console.log("\nTesting withColumns old record signature:");
    const dfWithColsRecord = df.withColumns({
    ...
    if (dfWithColsRecord.length !== 5 || dfWithColsRecord[0].salary_plus_500 !== 1500) {
        console.error("withColumns old record signature failed!");
        passed = false;
    }
    ...
    const dfNullsRes = dfNulls.select(
        ...
        $df.col("val_bool").and($df.col("val_bool").isNull()).alias("kleene_and")
    ).toDicts();
    ...
    const dfInnerJoined = dfJoinLeft.join({ other: dfJoinRight, on: "id", how: "inner" }).toDicts();
    ...
    const dfLeftJoined = dfJoinLeft.join({ other: dfJoinRight, on: "id", how: "left" }).toDicts();
    ...
    const dfOuterJoined = dfJoinLeft.join({ other: dfJoinRight, on: "id", how: "outer" }).toDicts();
    ...
    const dfGroupedUsability = dfUsability.groupBy("category").agg($df.col("val").sum().alias("sum_val")).toDicts();
    ...
    const dfUniqueUsability = dfUsability.unique("category").toDicts();
    ...
    const dfUnpivotUsability = dfUsability.unpivot({ idVars: "category", valueVars: "val" }).toDicts();
    ...
    const dfPivotUsability = dfUsability.pivot({ index: "category", columns: "category", values: "val" }).toDicts();
    ...
    try {
        ...
        const collectedInvalid = dfInvalid.toDicts();
    ...
    try {
        ...
        const dfRenameGuard = dfUsability.rename(undefined).toDicts();
        ...
        const dfSortGuard1 = dfUsability.sort(undefined).toDicts();
        const dfSortGuard2 = dfUsability.sort({} as any).toDicts();
        if (dfSortGuard1.length !== 3 || dfSortGuard2.length !== 3) {
            console.error("Sort guard failed!");
            passed = false;
        }

        // 14.3 toArray with null nameOrExpr returns []
        try {
            const arr = dfUsability.toArray(null as any);
            if (!Array.isArray(arr) || arr.length !== 0) {
                console.error("toArray(null) did not return []");
                passed = false;
            }
        } catch (e) {
            console.error("toArray(null) crashed:", e);
            passed = false;
        }
    } catch (e) {
