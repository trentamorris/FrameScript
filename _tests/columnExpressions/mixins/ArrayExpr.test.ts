...
try {
    ...
    const projected = df.select([
        ...
        $df.col("numbers").arr.filter($df.element().isNotNull()).alias("dropped_nulls_nums"),
    ...
    const projectedComplex = complexDf.select([
        ...
        $df.col("tags").arr.filter($df.element().isNotNull().and($df.element().str.startsWith("a"))).alias("a_tags"),
        ...
        // toStruct operations with upperBound and fields sequences
        $df.col("tags").arr.toStruct({ fields: ["t1", "t2"] }).alias("tags_struct_fields"),
        $df.col("tags").arr.toStruct().alias("tags_struct_default"),
        $df.col("tags").arr.toStruct({ upperBound: 4 }).alias("tags_struct_upper_bound"),
        $df.col("tags").arr.toStruct({ upperBound: 4, fields: (idx) => `idx_${idx}` }).alias("tags_struct_upper_bound_custom_fn")
    ]).toDicts() as any[];
    ...
    // Assert toStruct strategy outputs
    ...
    // tags_struct_default now uses max_width fallback, matching upperBound=4
    ...
    try {
        complexDf.select([
            $df.col("tags").arr.toStruct({ upperBound: 0 })
        ]).toDicts();
    } catch (e: any) {
    ...
    if (!threwZeroWidth) {
        throw new Error("Expected zero width toStruct to throw an error");
    }
    console.log("✓ toStruct width === 0 error guard passed");
    ...
    const dfEvalData = $df.data(evalTestData);

    // 1. Basic element-wise arithmetic: element().mul(2)
    const dfArithmetic = dfEvalData.select([
        $df.col("numbers").arr.eval($df.element().mul(2)).alias("arithmetic")
    ]);
    const resArithmetic = dfArithmetic.toDicts();
    ...
    const dfSum = dfEvalData.select([
        $df.col("numbers").arr.eval($df.element().sum()).alias("sum")
    ]);
    const resSum = dfSum.toDicts();
    ...
    const dfChained = dfEvalData.select([
        $df.col("numbers").arr.eval($df.element().sum().mul(10).add(1)).alias("chained")
    ]);
    const resChained = dfChained.toDicts();
    ...
    // 4. Element-wise string: element().str.toUpperCase()
    const dfString = dfEvalData.select([
        $df.col("tags").arr.eval($df.element().str.toUpperCase()).alias("upper")
    ]);
    const resString = dfString.toDicts();
    ...
    const dfNull = dfEvalData.select([
        $df.col("null_col").arr.eval($df.element().add(1)).alias("null_eval")
    ]);
    const resNull = dfNull.toDicts();
    ...
    const dfOver = $df.data([
        ...
        $df.col("numbers").arr.eval($df.element().sum().over($df.element())).alias("over_eval")
    ]);
    const resOver = dfOver.toDicts();
    ...
    const dfCustomOver = $df.data([
        ...
        $df.col("numbers").arr.eval($df.element().sum().over($df.element().mod(2))).alias("custom_over")
    ]);
    const resCustomOver = dfCustomOver.toDicts();
    ...
    console.log("✓ All array.eval tests passed successfully!");
    // ----------------------------------------------------
    // END ARRAY.EVAL TESTS
    // ----------------------------------------------------
