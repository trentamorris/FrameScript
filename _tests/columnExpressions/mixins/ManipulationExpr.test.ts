...
try {
    ...
    const projected = df.select([
        // 1. fillNull (scalar and expression)
        $df.col("val").fillNull({ value: 99 }).alias("val_filled_scalar"),
        $df.col("val").fillNull({ value: $df.col("other_val") }).alias("val_filled_expr"),
        $df.col("null_val").fillNull({ value: "fallback" }).alias("null_filled"),
        ...
        // 4. list any/all/contains/dropNulls
        ...
        $df.col("str_val").str.countMatches("apple").alias("str_matches_str"),
    ...
    // Test fill_nan, dropNulls, drop_nans, explode, implode
    ...
    // 1. DataFrame.explode
    const exploded = expDf.explode("list").toDicts();
    ...
    const explodedNoEmpty = expDf.explode("list", { emptyAsNull: false }).toDicts();
    ...
    const explodedNoNulls = expDf.explode("list", { keepNulls: false }).toDicts();
    ...
    const explodedNeither = expDf.explode("list", { emptyAsNull: false, keepNulls: false }).toDicts();
    ...
    const explodedExpr = expDf.explode($df.col("list")).toDicts();
    if (explodedExpr.length !== 5 || explodedExpr[0].list !== 10) throw new Error("explode with expression failed");

    const explodedList = expDf.explode(["list"]).toDicts();
    ...
    const explodedMulti = multiDf.explode(["list1", "list2"]).toDicts();
    ...
    const selectMatching = matchingDf.select([$df.col("id"), $df.col("list").arr.explode().alias("exploded_val")]).toDicts();
    ...
    // 3. col().arr.explode in withColumns expands columns and repeats existing columns
    const withColsExploded = expDf.withColumns(
        $df.col("list").arr.explode().alias("exploded_val")
    ).toDicts();
    console.log("withColsExploded results:");
    console.dir(withColsExploded);
    if (withColsExploded.length !== 5) {
        throw new Error(`Expected withColumns exploded height 5, got ${withColsExploded.length}`);
    }
    if (withColsExploded[0].id !== 1 || withColsExploded[0].exploded_val !== 10 || withColsExploded[0].tag !== "A") throw new Error("withColumns explode row 0 failed");
    if (withColsExploded[1].id !== 1 || withColsExploded[1].exploded_val !== 20 || withColsExploded[1].tag !== "A") throw new Error("withColumns explode row 1 failed");
    if (withColsExploded[2].id !== 2 || withColsExploded[2].exploded_val !== null || withColsExploded[2].tag !== "B") throw new Error("withColumns explode row 2 failed");
    if (withColsExploded[3].id !== 3 || withColsExploded[3].exploded_val !== null || withColsExploded[3].tag !== "C") throw new Error("withColumns explode row 3 failed");
    if (withColsExploded[4].id !== 4 || withColsExploded[4].exploded_val !== 30 || withColsExploded[4].tag !== "D") throw new Error("withColumns explode row 4 failed");

    // Test ShapeError on height mismatch in select/withColumns
    ...
    try {
        mismatchDf.withColumns(badExpr);
    } catch (e: any) {
        if (e instanceof ShapeError && e.message.includes("Column height mismatch")) {
            withColsThrew = true;
        }
    ...
    if (!withColsThrew) {
        throw new Error("Expected withColumns to throw ShapeError on height mismatch");
    }
    ...
    const seqRangeBroadcast = testSeqRangeDf.select([
        $df.seqRange("z", { mode: "constant" }).alias("broadcasted")
    ]).toDicts();
    if (seqRangeBroadcast.length !== 3) throw new Error("seqRange broadcast length mismatch");
    for (let i = 0; i < 3; i++) {
        if (seqRangeBroadcast[i].broadcasted !== "z") {
            throw new Error(`seqRange broadcast value mismatch at index ${i}`);
        }
    }
    ...
    const seqRangeExact = testSeqRangeDf.select([
        $df.seqRange("x", { n: 3, mode: "constant" }).alias("exact")
    ]).toDicts();
    if (seqRangeExact.length !== 3) throw new Error("seqRange exact length mismatch");
    for (let i = 0; i < 3; i++) {
        if (seqRangeExact[i].exact !== "x") {
            throw new Error(`seqRange exact value mismatch at index ${i}`);
        }
    }
    ...
    try {
        testSeqRangeDf.select([$df.seqRange("y", { n: 5, mode: "constant" })]);
    } catch (e: any) {
        if (e instanceof ShapeError && e.message.includes("Column height mismatch")) {
    ...
    if (!strictThrewLarger) {
        throw new Error("Expected seqRange to throw ShapeError when n > DataFrame height under strict: true");
    }

    // 4. Strict mode mismatch: n = 2 (smaller than df height 3)
    let strictThrewSmaller = false;
    try {
        testSeqRangeDf.select([$df.seqRange("y", { n: 2, mode: "constant" })]);
    } catch (e: any) {
        if (e instanceof ShapeError && e.message.includes("Column height mismatch")) {
    ...
    const seqRangeDtype = testSeqRangeDf.select([
        $df.seqRange(123.45, { n: 3, dtype: $df.DataType.Int32, mode: "constant" }).alias("val")
    ]).toDicts();
    for (let i = 0; i < 3; i++) {
        if (seqRangeDtype[i].val !== 123) {
            throw new Error(`seqRange dtype coercion failed at index ${i}, expected 123 got ${seqRangeDtype[i].val}`);
        }
    }

    // 6. Strict mode with name: sets output column name
    const seqRangeName = testSeqRangeDf.select([
        $df.seqRange("a", { name: "custom_name", mode: "constant" })
    ]).toDicts();
    if (seqRangeName[0].custom_name !== "a") {
        throw new Error("seqRange option name failed to rename column");
    }

    // 7. Non-strict mode: omitted n behaves like lit
    const seqRangeNonStrictBroadcast = testSeqRangeDf.select([
        $df.seqRange("z", { strict: false, mode: "constant" }).alias("val")
    ]).toDicts();
    for (let i = 0; i < 3; i++) {
        if (seqRangeNonStrictBroadcast[i].val !== "z") {
            throw new Error("seqRange non-strict broadcast failed");
        }
    }

    // 8. Non-strict mode: n matches DataFrame height
    const seqRangeNonStrictExact = testSeqRangeDf.select([
        $df.seqRange("x", { n: 3, strict: false, mode: "constant" }).alias("val")
    ]).toDicts();
    for (let i = 0; i < 3; i++) {
        if (seqRangeNonStrictExact[i].val !== "x") {
            throw new Error("seqRange non-strict exact failed");
        }
    }
    ...
    const seqRangeNonStrictTruncateDefault = testSeqRangeDf.select([
        $df.seqRange("a", { n: 5, strict: false, mode: "constant" }).alias("val")
    ]).toDicts();
    if (seqRangeNonStrictTruncateDefault.length !== 3) throw new Error("seqRange non-strict truncate default length failed");
    for (let i = 0; i < 3; i++) {
        if (seqRangeNonStrictTruncateDefault[i].val !== "a") {
            throw new Error("seqRange non-strict truncate default values failed");
        }
    }
    ...
    const seqRangeNonStrictTruncateExplicit = testSeqRangeDf.select([
        $df.seqRange("c", { n: 5, strict: false, truncate: true, pad: false, mode: "constant" }).alias("val")
    ]).toDicts();
    for (let i = 0; i < 3; i++) {
        if (seqRangeNonStrictTruncateExplicit[i].val !== "c") {
            throw new Error("seqRange non-strict truncate explicit failed");
        }
    }
    ...
    try {
        testSeqRangeDf.select([$df.seqRange("d", { n: 2, strict: false, truncate: true, pad: false, mode: "constant" })]);
    } catch (e: any) {
        if (e instanceof ShapeError && e.message.includes("Cannot truncate seqRange output")) {
            nonStrictTruncateSmallerThrew = true;
        }
    }
    if (!nonStrictTruncateSmallerThrew) {
        throw new Error("Expected non-strict seqRange to throw when padding is required but not allowed");
    }
    ...
    const seqRangeStartEndTruncated = testSeqRangeDf.select([
        $df.seqRange("a", { n: 2, strict: false, startIndex: 1, endIndex: 2, truncate: true, mode: "constant" }).alias("val")
    ]).toDicts();
    if (seqRangeStartEndTruncated[0].val !== null || seqRangeStartEndTruncated[1].val !== "a" || seqRangeStartEndTruncated[2].val !== null) {
        throw new Error("seqRange startIndex and endIndex with truncate failed");
    }
    ...
    const seqRangeStartEndPadded = testSeqRangeDf.select([
        $df.seqRange("b", { n: 1, strict: false, startIndex: 1, endIndex: 3, pad: true, padValue: "x", mode: "constant" }).alias("val")
    ]).toDicts();
    if (seqRangeStartEndPadded[0].val !== "x" || seqRangeStartEndPadded[1].val !== "b" || seqRangeStartEndPadded[2].val !== "x") {
        throw new Error("seqRange startIndex and endIndex with pad failed");
    }
