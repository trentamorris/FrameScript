...
try {
    ...
    const results = df.select([
        ...
        // isClose
        $df.col("val").isClose(10.500001, { relTol: 1e-5 }).alias("val_close"),
        $df.col("val").isClose(11.0).alias("val_not_close"),
        $df.col("val").isClose(NaN, { nansEqual: true }).alias("val_nan_close_eq"),
        $df.col("val").isClose(NaN, { nansEqual: false }).alias("val_nan_close_neq"),
        $df.col("val").isClose(Infinity).alias("val_inf_close"),

        // isDuplicated
        $df.col("duplicates").isDuplicated().alias("dup_mask"),

        // isUnique
        $df.col("duplicates").isUnique().alias("uniq_mask"),

        // isEmpty
        $df.col("name").isEmpty().alias("name_empty"),
        $df.col("flags").isEmpty().alias("flags_empty"),

        // not
        $df.col("id").eq(1).not().alias("not_id_eq_1"),

        // Null and NaN checks
        $df.col("val").isNull().alias("val_null"),
        $df.col("val").isNotNull().alias("val_not_null"),
        $df.col("val").isNan().alias("val_nan"),
        $df.col("val").isNotNan().alias("val_not_nan"),
        $df.col("val").isFinite().alias("val_finite"),
        $df.col("val").isInfinite().alias("val_infinite"),

        // isIn / notIn
        $df.col("duplicates").isIn(["A", "C"]).alias("dup_in_AC"),
        $df.col("duplicates").notIn(["A", "C"]).alias("dup_not_in_AC")
    ]).toDicts() as any[];
    ...
    console.log("✓ isEmpty checks passed");
    ...
    console.log("✓ isIn/notIn checks passed");
    ...
    // 3. Robust/edge-case tests for isUnique, isDuplicated, is_n_distinct, and isIn
    const edgeData = [
        {
    ...
    const edgeResults = edgeDf.select([
        $df.col("dateVal").isUnique().alias("date_unique"),
        $df.col("dateVal").isDuplicated().alias("date_duplicated"),
        $df.col("binaryVal").isUnique().alias("binary_unique"),
        $df.col("binaryVal").isDuplicated().alias("binary_duplicated"),
        $df.col("numVal").isUnique().alias("num_unique"),
        $df.col("numVal").isDuplicated().alias("num_duplicated"),
        $df.col("nullVal").isUnique().alias("null_unique"),
        $df.col("nullVal").isDuplicated().alias("null_duplicated"),
        $df.col("dateVal").isIn([new Date("2026-06-01T12:00:00.000Z"), new Date("2026-06-03T12:00:00.000Z")]),
        $df.col("binaryVal").isIn([new Uint8Array([1, 2, 3])]),
        $df.col("numVal").isIn([NaN]),
        $df.col("listVal").isEmpty().alias("list_empty_default"),
        $df.col("listVal").isEmpty({ ignoreNulls: true }).alias("list_empty_ignore_nulls")
    ]).toDicts() as any[];
    ...
    // isEmpty with ignoreNulls comparisons
