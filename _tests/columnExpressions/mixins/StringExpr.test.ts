...
try {
    ...
    const projected = df.select([
        // Basic conversions
        $df.col("phrase").str.toLowerCase().alias("lower"),
        $df.col("phrase").str.toUpperCase().alias("upper"),
        $df.col("phrase").str.toTitleCase().alias("title"),
        $df.col("phrase").str.reverse().alias("reversed"),

        // Lengths
        $df.col("phrase").str.lenChars().alias("len_c"),
        $df.col("phrase").str.lenBytes().alias("len_b"),
        ...
        // Stripping
        $df.col("name").str.stripChars().alias("stripped_ws"),
        $df.col("name").str.stripChars(" -").alias("stripped_chars"),
        $df.col("name").str.stripCharsStart(undefined).alias("stripped_start_ws"),
        $df.col("name").str.stripCharsEnd().alias("stripped_end_ws"),
        $df.col("name").str.stripChars("-", { trimFirst: true }).alias("stripped_chars_trim_first"),
        $df.col("name").str.stripChars("-", { maxScanStart: 3, maxScanEnd: 3 }).alias("stripped_chars_offset_3"),
        $df.col("name").str.stripChars(/[a-zA-Z]/, { maxScanStart: 10, maxScanEnd: 10 }).alias("stripped_chars_regex"),
        ...
        $df.col("phrase").str.containsAny(["missing", "awesome"]).alias("contains_any_match"),
        $df.col("phrase").str.containsAny(["foo", "bar"]).alias("contains_any_none"),
    ...
    // Tests for containsAny robustness and stateful regexes
    ...
    const containsEdgeRes = containsEdgeDf.select([
        $df.col("text").str.containsAny([globalRegex]).alias("global_regex_match"),
        $df.col("text").str.containsAny("apple" as any).alias("non_array_match"),
        $df.col("text").str.containsAny([null as any, "pie"]).alias("null_element_match")
    ]).toDicts() as any[];

    if (containsEdgeRes[0].global_regex_match !== true || containsEdgeRes[1].global_regex_match !== true) {
        throw new Error(`Stateful regex containsAny failed across rows: got ${JSON.stringify(containsEdgeRes)}`);
    }
    if (containsEdgeRes[0].non_array_match !== true) {
        throw new Error(`Non-array containsAny failed: expected true, got ${containsEdgeRes[0].non_array_match}`);
    }
    if (containsEdgeRes[0].null_element_match !== true || containsEdgeRes[1].null_element_match !== false) {
        throw new Error(`Null element containsAny failed: got ${JSON.stringify(containsEdgeRes)}`);
    }
    ...
    try {
        encDecDf.select([$df.col("invalid_hex").str.decode({ encoding: "hex", strict: true })]).toDicts();
    } catch {
        strictHexThrew = true;
    }
    ...
    try {
        encDecDf.select([$df.col("invalid_b64").str.decode({ encoding: "base64", strict: true })]).toDicts();
    } catch {
        strictB64Threw = true;
    }
    ...
    const matchRes = matchDf.select([
        ...
        $df.col("text").str.countMatches(/./g).alias("count_regex_dot"),
        $df.col("text").str.countMatches(".", { literal: true }).alias("count_literal_dot"),
        $df.col("text").str.countMatches(/\d+/g).alias("count_regex_digits"),
        $df.col("text").str.countMatches("").alias("count_empty_pat")
    ]).toDicts() as any[];

    if (matchRes[0].count_a !== 3) throw new Error(`countMatches 'a' failed: ${matchRes[0].count_a}`);
    if (matchRes[1].count_regex_dot !== 19) throw new Error(`countMatches regex dot failed: ${matchRes[1].count_regex_dot}`);
    if (matchRes[1].count_literal_dot !== 2) throw new Error(`countMatches literal dot failed: ${matchRes[1].count_literal_dot}`);
    if (matchRes[2].count_regex_digits !== 2) throw new Error(`countMatches regex digits failed: ${matchRes[2].count_regex_digits}`);
    if (matchRes[3].count_empty_pat !== 5) throw new Error(`countMatches empty pat failed: ${matchRes[3].count_empty_pat}`);
    if (matchRes[4].count_a !== null) throw new Error(`countMatches null input failed: ${matchRes[4].count_a}`);

    // Detailed countMatches edge case battery
    ...
    const advMatchRes = advMatchDf.select([
        $df.col("str").str.countMatches("aa").alias("non_overlap"),
        $df.col("str").str.countMatches("🚀").alias("emoji_count"),
        $df.col("str").str.countMatches("$[0-9]+", true).alias("literal_bool"),
        $df.col("str").str.countMatches(/foo/i).alias("non_global_regex"),
        $df.col("str").str.countMatches("\n").alias("newline_count"),
        $df.col("str").str.countMatches("abc").alias("empty_str_input"),
        $df.col("str").str.countMatches(123 as any).alias("num_coerced_pat"),
        $df.col("str").str.countMatches(null as any).alias("null_pat")
    ]).toDicts() as any[];
    ...
    const extremeRes = extremeDf.select([
        $df.col("str").str.countMatches(dirtyRegex).alias("dirty_regex_count"),
        $df.col("str").str.countMatches(/^foo/m).alias("multiline_regex"),
        $df.col("str").str.countMatches(/a.b/s).alias("dotall_regex"),
        $df.col("str").str.countMatches(/(?=a)/g).alias("lookahead_regex"),
        $df.col("str").str.countMatches("$1$2", { literal: true }).alias("regex_special_lit"),
        $df.col("str").str.countMatches("a").alias("large_repeat_50k"),
        $df.col("str").str.countMatches("\t").alias("tab_count"),
        $df.col("str").str.countMatches(-3.14 as any).alias("float_pat"),
        $df.col("str").str.countMatches(100n as any).alias("bigint_pat"),
        $df.col("str").str.countMatches("a").alias("boxed_string")
    ]).toDicts() as any[];
    ...
    const extractRes = extractDf.select([
        // --- extractAll ---
        $df.col("text").str.extractAll(/\d+/).alias("all_nums"),
        $df.col("text").str.extractAll(/([a-z]+)_(\d+)/, { groupIndex: 2 }).alias("all_group2"),
        $df.col("null_text").str.extractAll(/\d+/).alias("allNull"),

        // --- extractGroups ---
        $df.col("code").str.extractGroups(/(?<id_key>id:\d+)-(?<name_key>name:\w+)/).alias("named_groups"),
        $df.col("code").str.extractGroups(/(\w+):(\d+)/).alias("positional_groups"),
        $df.col("text").str.extractGroups(/no_match_pattern/).alias("no_match_groups"),
        $df.col("null_text").str.extractGroups(/\w+/).alias("null_groups"),

        // --- extractMany ---
        $df.col("text").str.extractMany([/\d+/, /item/, /missing/]).alias("many_default"),
        $df.col("text").str.extractMany([/USER_\d+/, /ITEM_\d+/], { asciiCaseInsensitive: true }).alias("many_group1"),
        $df.col("null_text").str.extractMany([/\d+/]).alias("many_null"),

        // --- extract: options object ---
        $df.col("code").str.extract(/(\w+):(\d+)/, { groupIndex: 2 }).alias("extract_opt"),
        $df.col("text").str.extractAll(/([a-z]+)_(\d+)/, { groupIndex: 2 }).alias("all_group2_opt"),
        $df.col("text").str.extractMany([/USER_\d+/, /ITEM_\d+/], { asciiCaseInsensitive: true }).alias("many_group1_opt"),
        ...
        // extractAll: string pattern (literal) → all full matches
        $df.col("text").str.extractAll("_").alias("extract_all_str"),
        // extractAll: groupIndex 0 → full match per occurrence
        $df.col("text").str.extractAll(/[a-z]+_\d+/, { groupIndex: 0 }).alias("extract_all_group0"),
        // extractAll: asciiCaseInsensitive
        $df.col("text").str.extractAll(/USER_\d+/, { asciiCaseInsensitive: true }).alias("extract_all_case"),
        // extractAll: no match → empty array (not null)
        $df.col("code").str.extractAll(/ZZZNOMATCH/).alias("extract_all_no_match"),
        // extractAll: null pattern guard → null
        $df.col("code").str.extractAll(null as any).alias("extract_all_null_pattern"),

        // extractGroups: no capture groups → only "0" key (full match)
        $df.col("code").str.extractGroups(/\d+/).alias("groups_no_captures"),
        // extractGroups: mixed named + positional
        $df.col("code").str.extractGroups(/(?<word>\w+):(\d+)/).alias("groups_mixed"),
        // extractGroups: includes "0" key (full match)
        $df.col("code").str.extractGroups(/(\w+):(\d+)/).alias("groups_has_zero"),

        // extractMany: empty patterns array → empty result array
        $df.col("text").str.extractMany([]).alias("many_empty"),
        // extractMany: single non-array pattern (auto-wrapped)
        $df.col("text").str.extractMany([/\d+/]).alias("many_single"),
        // extractMany: groupIndex captures group 1
        $df.col("text").str.extractMany([/([a-z]+)_(\d+)/], { groupIndex: 1 }).alias("many_group1_cap"),
        // extractMany: all patterns miss → all nulls
        $df.col("text").str.extractMany([/ZZZA/, /ZZZB/]).alias("many_all_null"),
        // extractMany: null patterns guard → null
        $df.col("text").str.extractMany(null as any).alias("many_null_patterns")
    ]).toDicts() as any[];
    ...
    if (JSON.stringify(r0Extract.all_nums) !== JSON.stringify(["123", "456"])) {
        throw new Error(`extractAll failed row 0 all_nums: ${JSON.stringify(r0Extract.all_nums)}`);
    }
    if (JSON.stringify(r0Extract.all_group2) !== JSON.stringify(["123", "456"])) {
        throw new Error(`extractAll failed row 0 all_group2: ${JSON.stringify(r0Extract.all_group2)}`);
    }
    if (JSON.stringify(r0Extract.all_group2_opt) !== JSON.stringify(["123", "456"])) {
        throw new Error(`extractAll options object failed: ${JSON.stringify(r0Extract.all_group2_opt)}`);
    }
    if (r0Extract.allNull !== null) throw new Error(`extractAll failed null_text: ${r0Extract.allNull}`);

    // extractGroups: check named keys (result also includes "0", "1", "2" positional keys)
    const ng = r0Extract.named_groups;
    if (ng?.id_key !== "id:100" || ng?.name_key !== "name:alice") {
        throw new Error(`extractGroups failed row 0 named_groups: ${JSON.stringify(ng)}`);
    }
    // extractGroups: check positional keys (result includes "0" for full match)
    const pg = r0Extract.positional_groups;
    if (pg?.["1"] !== "id" || pg?.["2"] !== "100" || pg?.["0"] !== "id:100") {
        throw new Error(`extractGroups failed row 0 positional_groups: ${JSON.stringify(pg)}`);
    }
    if (r0Extract.no_match_groups !== null) throw new Error(`extractGroups failed no match: ${r0Extract.no_match_groups}`);
    if (r0Extract.null_groups !== null) throw new Error(`extractGroups failed null string: ${r0Extract.null_groups}`);

    if (JSON.stringify(r0Extract.many_default) !== JSON.stringify(["123", "item", null])) {
        throw new Error(`extractMany failed row 0 many_default: ${JSON.stringify(r0Extract.many_default)}`);
    }
    if (JSON.stringify(r0Extract.many_group1) !== JSON.stringify(["user_123", "item_456"])) {
        throw new Error(`extractMany failed row 0 many_group1: ${JSON.stringify(r0Extract.many_group1)}`);
    }
    if (JSON.stringify(r0Extract.many_group1_opt) !== JSON.stringify(["user_123", "item_456"])) {
        throw new Error(`extractMany options object failed: ${JSON.stringify(r0Extract.many_group1_opt)}`);
    }
    if (r0Extract.many_null !== null) throw new Error(`extractMany failed null string: ${r0Extract.many_null}`);
    ...
    // extractAll: string pattern → ["_", "_", "_"] (3 underscores in "user_123_item_456")
    if (JSON.stringify(r0Extract.extract_all_str) !== JSON.stringify(["_", "_", "_"])) {
        throw new Error(`extractAll string pattern failed: ${JSON.stringify(r0Extract.extract_all_str)}`);
    }
    // extractAll: groupIndex 0 → full match per occurrence
    if (JSON.stringify(r0Extract.extract_all_group0) !== JSON.stringify(["user_123", "item_456"])) {
        throw new Error(`extractAll groupIndex:0 failed: ${JSON.stringify(r0Extract.extract_all_group0)}`);
    }
    // extractAll: asciiCaseInsensitive
    if (JSON.stringify(r0Extract.extract_all_case) !== JSON.stringify(["user_123"])) {
        throw new Error(`extractAll asciiCaseInsensitive failed: ${JSON.stringify(r0Extract.extract_all_case)}`);
    }
    // extractAll: no match → empty array (not null)
    if (JSON.stringify(r0Extract.extract_all_no_match) !== JSON.stringify([])) {
        throw new Error(`extractAll no match failed: ${JSON.stringify(r0Extract.extract_all_no_match)}`);
    }
    // extractAll: null pattern → null
    if (r0Extract.extract_all_null_pattern !== null) throw new Error(`extractAll null pattern failed: ${r0Extract.extract_all_null_pattern}`);

    // extractGroups: no capture groups → only "0" key
    const gnc = r0Extract.groups_no_captures;
    if (gnc?.["0"] !== "100" || Object.keys(gnc).length !== 1) {
        throw new Error(`extractGroups no captures failed: ${JSON.stringify(gnc)}`);
    }
    // extractGroups: mixed named + positional
    const gm = r0Extract.groups_mixed;
    if (gm?.word !== "id" || gm?.["2"] !== "100") {
        throw new Error(`extractGroups mixed failed: ${JSON.stringify(gm)}`);
    }
    // extractGroups: "0" key present (full match)
    if (r0Extract.groups_has_zero?.["0"] !== "id:100") {
        throw new Error(`extractGroups "0" key failed: ${JSON.stringify(r0Extract.groups_has_zero)}`);
    }

    // extractMany: empty patterns array → empty array
    if (JSON.stringify(r0Extract.many_empty) !== JSON.stringify([])) {
        throw new Error(`extractMany empty patterns failed: ${JSON.stringify(r0Extract.many_empty)}`);
    }
    // extractMany: single pattern
    if (JSON.stringify(r0Extract.many_single) !== JSON.stringify(["123"])) {
        throw new Error(`extractMany single pattern failed: ${JSON.stringify(r0Extract.many_single)}`);
    }
    // extractMany: groupIndex:1 → first capture group
    if (JSON.stringify(r0Extract.many_group1_cap) !== JSON.stringify(["user"])) {
        throw new Error(`extractMany groupIndex:1 failed: ${JSON.stringify(r0Extract.many_group1_cap)}`);
    }
    // extractMany: all miss → all null
    if (JSON.stringify(r0Extract.many_all_null) !== JSON.stringify([null, null])) {
        throw new Error(`extractMany all miss failed: ${JSON.stringify(r0Extract.many_all_null)}`);
    }
    // extractMany: null patterns → null
    if (r0Extract.many_null_patterns !== null) throw new Error(`extractMany null patterns failed: ${r0Extract.many_null_patterns}`);

    // Row 1 assertions
    const r1Extract = extractRes[1];
    if (JSON.stringify(r1Extract.all_nums) !== JSON.stringify([])) {
        throw new Error(`extractAll failed row 1 no matches: ${JSON.stringify(r1Extract.all_nums)}`);
    }
    ...
    // Row 1: extractAll string pattern on "no numbers here" → []
    if (JSON.stringify(r1Extract.extract_all_str) !== JSON.stringify([])) {
        throw new Error(`extractAll string pattern row1 failed: ${JSON.stringify(r1Extract.extract_all_str)}`);
    }
    // Row 1: extractMany empty → []
    if (JSON.stringify(r1Extract.many_empty) !== JSON.stringify([])) {
        throw new Error(`extractMany empty row1 failed: ${JSON.stringify(r1Extract.many_empty)}`);
    }
    ...
    const advEdgeRes = advEdgeDf.withColumns([
        // 1. Zero-width match (boundary \b and empty string regex)
        $df.col("boundary_str").str.extractAll(/\b/).alias("zero_width_all"),
        $df.col("empty_str").str.extract(/^/, { groupIndex: 0 }).alias("zero_width_start"),
        $df.col("empty_str").str.extractAll(/^/).alias("zero_width_start_all"),
        ...
        $df.col("unicode_str").str.extractAll(/[\p{Extended_Pictographic}]/u).alias("unicode_emoji_all"),
        ...
        $df.col("pattern_str").str.extractAll(new RegExp("\\d+", "gy")).alias("sticky_flag_extract_all"),
        // 4. Overlapping patterns and negative group indices in extractMany
        $df.col("pattern_str").str.extractMany([/\d+/, /[a-z]+/], { groupIndex: -1 }).alias("extract_many_neg_group"),
        // 5. extractGroups with zero-width / non-matching optional group
        $df.col("pattern_str").str.extractGroups(/(?<letters>[a-z]+)(?<digits>\d+)?(?<optional>ZZZ)?/).alias("groups_optional_miss")
    ]).toDicts() as any[];
    ...
    // Assert negative groupIndex in extractMany (-1 gets last group or fallback if negative out-of-bounds)
    ...
    // Assert optional unmatched group in extractGroups is null
    ...
    // 6. extractMany with leftmost: true and overlapping: true test
    const optTestDf = $df.data({ text: ["user_123_prod"] });
    const leftmostRes = optTestDf.withColumns([
        $df.col("text").str.extractMany([/user_\d+/, /123/], { leftmost: true }).alias("many_leftmost"),
        $df.col("text").str.extractMany([/user_\d+/, /123/], { overlapping: true }).alias("many_overlapping")
    ]).toDicts() as any[];

    if (JSON.stringify(leftmostRes[0].many_leftmost) !== JSON.stringify(["user_123", null])) {
        throw new Error(`extractMany leftmost failed: ${JSON.stringify(leftmostRes[0].many_leftmost)}`);
    }
    if (JSON.stringify(leftmostRes[0].many_overlapping) !== JSON.stringify(["user_123", "123"])) {
        throw new Error(`extractMany overlapping failed: ${JSON.stringify(leftmostRes[0].many_overlapping)}`);
    }

    // Assert InvalidArgumentError when overlapping: true and leftmost: true are both specified
    let invalidArgThrown = false;
    try {
        optTestDf.withColumns([
            $df.col("text").str.extractMany([/user_\d+/], { overlapping: true, leftmost: true }).alias("err")
        ]).toDicts();
    } catch (err: any) {
    ...
    if (!invalidArgThrown) {
        throw new Error("extractMany failed to throw InvalidArgumentError when both overlapping and leftmost are true");
    }
    ...
    const joinRes = joinDf.withColumns([
    ...
    const normRes = normDf.withColumns([
    ...
    // ----------------------------------------------------
    // START JSONPATH EDGE CASES TESTS
    // ----------------------------------------------------
    const dfJsonPathValid = $df.data({
        json_str: [
    ...
    const resJson = dfJsonPathValid.withColumns([
        $df.col("json_str").str.jsonPathMatch("$").alias("root"),
        $df.col("json_str").str.jsonPathMatch("$.user.name").alias("name"),
        $df.col("json_str").str.jsonPathMatch("$['user']['name']").alias("single_quoted_name"),
        $df.col("json_str").str.jsonPathMatch('$["user"]["name"]').alias("double_quoted_name"),
        $df.col("json_str").str.jsonPathMatch("$.user.age").alias("age"),
        $df.col("json_str").str.jsonPathMatch("$.active").alias("active"),
        $df.col("json_str").str.jsonPathMatch("$.user.tags[0]").alias("first_tag"),
        $df.col("json_str").str.jsonPathMatch("$.user.tags[-1]").alias("last_tag"),
        $df.col("json_str").str.jsonPathMatch("$.user.tags[:2]").alias("tags_slice"),
        $df.col("json_str").str.jsonPathMatch("$..tags").alias("recursive_tags"),
        $df.col("json_str").str.jsonPathMatch("$..name").alias("recursive_name"),
        $df.col("json_str").str.jsonPathMatch("$.user.*").alias("user_wildcard"),
        $df.col("json_str").str.jsonPathMatch("$.missing").alias("missing")
    ]);

    const jsonRecords = resJson.toDicts() as any[];

    if (jsonRecords[0].name !== "Alice") throw new Error("Row 0 name failed");
    if (jsonRecords[0].single_quoted_name !== "Alice") throw new Error("Row 0 single quoted name failed");
    if (jsonRecords[0].double_quoted_name !== "Alice") throw new Error("Row 0 double quoted name failed");
    if (jsonRecords[0].age !== "30") throw new Error("Row 0 age failed");
    if (jsonRecords[0].active !== "true") throw new Error("Row 0 active failed");
    if (jsonRecords[0].first_tag !== "admin") throw new Error("Row 0 first tag failed");
    if (jsonRecords[0].last_tag !== "dev") throw new Error("Row 0 last tag failed");
    if (jsonRecords[0].tags_slice !== "admin") throw new Error("Row 0 tags slice failed");
    if (jsonRecords[0].recursive_name !== "Alice") throw new Error("Row 0 recursive name failed");
    if (jsonRecords[0].missing !== null) throw new Error("Row 0 missing failed");

    if (jsonRecords[1].name !== "Bob") throw new Error("Row 1 name failed");
    if (jsonRecords[1].last_tag !== "tester") throw new Error("Row 1 last tag failed");
    if (jsonRecords[1].active !== "false") throw new Error("Row 1 active failed");
    if (jsonRecords[2].name !== null) throw new Error("Row 2 null row failed");

    console.log("✓ All .str.jsonPathMatch tests passed successfully!");
    // ----------------------------------------------------
    // END JSONPATH EDGE CASES TESTS
    // ----------------------------------------------------
