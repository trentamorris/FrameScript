...
const schema = {
    ...
    name: $df.Utf8,
    score: $df.Int32,
    weight: $df.Decimal(10, 2),
    is_active: $df.Boolean
};
...
try {
    ...
    const r1 = df.select([$df.col($df.Int32)]).toDicts();
    ...
    const r2 = df.select([$df.col($df.Utf8)]).toDicts();
    ...
    const r3 = df.select([$df.col($df.Decimal)]).toDicts();
    ...
    const r5 = df.select([$df.col([$df.Int32, $df.Utf8])]).toDicts();
    ...
    const r6 = df.select([$df.col([$df.Utf8, $df.Int32])]).toDicts();
    ...
    const r7 = df.select([$df.col([$df.Decimal, $df.Utf8])]).toDicts();
