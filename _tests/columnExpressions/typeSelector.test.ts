...
try {
    ...
    const r1 = df.select([$df.col($df.DataType.Int32)]).toDicts();
    ...
    const r2 = df.select([$df.col($df.DataType.Utf8)]).toDicts();
    ...
    const r3 = df.select([$df.col($df.DataType.Decimal)]).toDicts();
    if (!("weight" in r3[0]) || "id" in r3[0]) {
        throw new Error("r3 select Decimal failed: " + JSON.stringify(r3));
    }

    // 4. Using withColumns on specific types (e.g. multiply all Int32 columns by 10)
    const r4 = df.withColumns(
        $df.col($df.DataType.Int32).mul(10)
    ).toDicts();
    if (r4[0].id !== 10 || r4[0].score !== 950 || r4[0].name !== "Alice") {
        throw new Error("r4 withColumns Int32 failed: " + JSON.stringify(r4));
    }

    // 5. Select all Int32 AND Utf8 columns: id, name, score
    const r5 = df.select([$df.col([$df.DataType.Int32, $df.DataType.Utf8])]).toDicts();
    ...
    const r6 = df.select([$df.col([$df.DataType.Utf8, $df.DataType.Int32])]).toDicts();
    ...
    const r7 = df.select([$df.col([$df.DataType.Decimal, $df.DataType.Utf8])]).toDicts();
