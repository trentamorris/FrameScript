...
try {
    ...
    // 1. Test standard writeCsv (string output, auto-quotes, headers)
    const csvStr = df.writeCsv();
    if (typeof csvStr !== "string") throw new Error("writeCsv output was not a string");
    ...
    const noHeaderCSV = df.writeCsv(undefined, { includeHeader: false });
    ...
    const alwaysQuoted = df.writeCsv(undefined, { quoteStyle: "always" });
    ...
    const neverQuoted = df.writeCsv(undefined, { quoteStyle: "never" });
    ...
    const nonNumericQuoted = df.writeCsv(undefined, { quoteStyle: "non_numeric" });
    ...
    // 5. Test nullValue
    const customNulls = df.writeCsv(undefined, { nullValue: "N/A" });
    ...
    const preciseQuoted = df.writeCsv(undefined, { numericFormatOptions: { minimumFractionDigits: 2, maximumFractionDigits: 2 } });
    if (!preciseQuoted.includes(",95.12,")) {
        throw new Error("numericFormatOptions: custom decimal places failed");
    }

    // 7. Test includeBom
    const bomStr = df.writeCsv(undefined, { includeBom: true });
    if (!bomStr.startsWith("\ufeff")) {
        throw new Error("includeBom: missing Byte Order Mark");
    }

    // 8. Test lineTerminator
    const crlfStr = df.writeCsv(undefined, { lineTerminator: "\r\n" });
    if (!crlfStr.includes("\r\n")) {
    ...
    df.writeCsv(tempFilePath);
    if (!fs.existsSync(tempFilePath)) throw new Error("writeCsv to file path failed: file does not exist");
    const fileContent = fs.readFileSync(tempFilePath, "utf8");
    if (!fileContent.includes("Alice") || !fileContent.includes("Bob")) {
        throw new Error("writeCsv file content mismatch");
    }
    fs.unlinkSync(tempFilePath);
    ...
    if (!errorThrown) throw new Error("writeCsv failed to throw on invalid file argument");

    // 12. Test replacerOptions: onBigInt override in writeCsv
    const dfBigInt = new DataFrame([{ id: 1, val: 100n }]);
    const csvBigInt = dfBigInt.writeCsv(undefined, {
        replacerOptions: {
            onBigInt: (b) => `${b}n`
        }
    });
    if (!csvBigInt.includes("100n")) {
        throw new Error("writeCsv onBigInt override failed");
    }

    // 13. Test replacerOptions: onSet override in writeCsv
    const dfSet = new DataFrame([{ id: 1, val: new Set(["foo", "bar"]) }]);
    const csvSet = dfSet.writeCsv(undefined, {
        replacerOptions: {
            onSet: (s) => Array.from(s).join("|")
        }
    });
    if (!csvSet.includes("foo|bar")) {
        throw new Error("writeCsv onSet override failed");
    }

    // 14. Test replacerOptions: onCustom override for arbitrary classes in writeCsv
    class CustomVal {
        constructor(public text: string) {}
    }
    const dfCustom = new DataFrame([{ id: 1, val: new CustomVal("testing") }]);
    const csvCustom = dfCustom.writeCsv(undefined, {
        replacerOptions: {
            onCustom: (_, v) => (v instanceof CustomVal ? `Custom[${v.text}]` : v)
        }
    });
    if (!csvCustom.includes("Custom[testing]")) {
        throw new Error("writeCsv onCustom override failed");
    }
    ...
    const csvNested = dfNested.writeCsv(undefined, {
        replacerOptions: {
            onBigInt: (b) => `${b}Big`,
    ...
    if (!csvNested.includes('"{""innerBig"":""12345Big"",""innerSet"":[""aSet""]}"')) {
        throw new Error(`writeCsv nested replacerOptions failed: ${csvNested}`);
    }
    ...
    const customCSV = dfCustomDelim.writeCsv(undefined, { separator: "|", quoteChar: "'" });
    if (!customCSV.includes("'1|2'|'it''s'")) {
        throw new Error(`writeCsv custom separator & quoteChar failed: ${customCSV}`);
    }
    ...
    // Test 1: UTF-8 BOM Handling in parseCSV and writeCsv
    const bomCsv = "\uFEFFa,b\n1,2";
    ...
    const bomWritten = dfBom.writeCsv(undefined, { includeBom: true });
    if (!bomWritten.startsWith("\uFEFFx\n10")) {
        throw new Error("Edge Case 1 failed: includeBom in writeCsv");
    }
    ...
    streamDf.writeCsv({ write: (r: string) => streamedRows.push(r) });
    if (streamedRows.length !== 3 || streamedRows[0] !== "a,b" || streamedRows[1] !== "\n1,x" || streamedRows[2] !== "\n2,y") {
        throw new Error("Edge Case 7 failed: writeCsv streaming write method target");
    }
    ...
    const tsvOut = tsvData.writeCsv(undefined, { separator: "\t" });
    ...
    const customTermCsv = customLineTermDf.writeCsv(undefined, { lineTerminator: "\r\n" });
    ...
    // Test 16: writeCsv with custom date, time, and datetime formatting
    const temporalDf = new DataFrame([{ d: new Date("2026-08-17T14:30:45Z") }]);
    const formattedTemporal = temporalDf.writeCsv(undefined, { datetimeFormat: "%Y/%m/%d %H:%M:%S" });
    ...
    const emptyColsCsv = emptyColsDf.writeCsv(undefined, { quoteStyle: "necessary" });
    ...
    const customQuoteCharCsv = customQuoteCharDf.writeCsv(undefined, { quoteChar: "'", quoteStyle: "non_numeric" });
    ...
    streamBomDf.writeCsv({ write: (r: string) => streamBomRows.push(r) }, { includeBom: true });
    ...
    const headerSpecialCsv = headerSpecialDf.writeCsv();
    ...
    const noQuoteCsv = noQuoteDf.writeCsv(undefined, { quoteStyle: "never" });
    if (noQuoteCsv !== "a,b\nfoo,bar,100") {
    ...
    const emptyDfCsv = emptyDf.writeCsv();
    if (emptyDfCsv !== "a,b") {
    ...
    const emptyDfNoHeader = emptyDf.writeCsv(undefined, { includeHeader: false });
    if (emptyDfNoHeader !== "") {
