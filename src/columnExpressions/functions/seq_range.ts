    ...
    expr._ops.push((vArray) => {
        ...
        if (strict) {
            if (specifiedHeight !== targetHeight) {
                throw new ShapeError(`Column height mismatch: seq_range length ${specifiedHeight} does not match DataFrame height ${targetHeight}`);
            }
        } else {
            ...
            if (pad && !truncate && specifiedHeight > sliceWidth) {
                throw new ShapeError(`Cannot pad seq_range output: specified length ${specifiedHeight} starting at index ${safeStart} exceeds slice width ${sliceWidth} (requires truncation).`);
            }
            if (truncate && !pad && specifiedHeight < sliceWidth) {
                throw new ShapeError(`Cannot truncate seq_range output: specified length ${specifiedHeight} starting at index ${safeStart} is less than slice width ${sliceWidth} (requires padding).`);
            }
        }
